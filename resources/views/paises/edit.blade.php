...
<div class="max-w-2xl mx-auto p-4 lg:p-6">
    <div class="glass rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-military-800 to-military-700 px-6 py-6">
            <h1 class="text-3xl font-bold text-white">✏️ Editar País</h1>
            <p class="text-military-200 mt-1">{{ $paise->nombre }}</p>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('paises.update', $paise) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $paise->nombre) }}" required
            ...
            <div>
                ...
                <input type="text" id="codigo" name="codigo" value="{{ old('codigo', $paise->codigo) }}" maxlength="3" required
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="4"
                    class="w-full bg-dark-700 border border-military-600 rounded-lg px-4 py-2 text-black placeholder-gray-500 focus:outline-none focus:border-military-400 focus:ring-2 focus:ring-military-400/30 transition-all @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $paise->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
