...
<div class="max-w-4xl mx-auto p-4 lg:p-6">
    <div class="glass rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-military-800 to-military-700 px-6 py-8">
            <h1 class="text-4xl font-bold text-white mb-2">{{ $paise->nombre }}</h1>
            <p class="text-military-200">Código: <span class="font-semibold text-military-100">{{ $paise->codigo }}</span></p>
        </div>
        ...
        <div class="px-6 py-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-300 leading-relaxed">{{ $paise->descripcion ?? 'Sin descripción disponible' }}</p>
            </div>

            <!-- Tanques de este País -->
            <div class="mb-8">
                <h2 class="text-xl font-bold text-white mb-4 flex items-center gap-2">
                    <span class="text-2xl"></span> Tanques de este País
                </h2>
                @if ($paise->tanques->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach ($paise->tanques as $tanque)
        ...
        <div class="bg-dark-800 px-6 py-6 flex gap-3 justify-end border-t border-military-700">
            ...
            <a href="{{ route('paises.edit', $paise) }}" class="px-6 py-2 rounded-lg font-medium bg-military-600 hover:bg-military-500 text-white transition-colors">
                Editar
            </a>
        </div>
    </div>
