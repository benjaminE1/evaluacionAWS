...
<div class="max-w-4xl mx-auto p-4 lg:p-6">
    <div class="glass rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-military-800 to-military-700 px-6 py-6">
            <h1 class="text-3xl font-bold text-white">➕ Nuevo Tanque</h1>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('tanques.store') }}" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select id="fabricante_id" name="fabricante_id" required
                        ...
                        @foreach ($fabricantes as $fabricante)
                            <option value="{{ $fabricante->id }}" {{ old('fabricante_id') == $fabricante->id ? 'selected' : '' }}>{{ $fabricante->nombre }}</option>
                        @endforeach
                    </select>
                    @error('fabricante_id')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="combustible_id" name="combustible_id" required
                        ...
                        @foreach ($combustibles as $combustible)
                            <option value="{{ $combustible->id }}" {{ old('combustible_id') == $combustible->id ? 'selected' : '' }}>{{ $combustible->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" id="año_produccion" name="año_produccion" value="{{ old('año_produccion') }}" min="1900" max="2100"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" id="blindaje" name="blindaje" value="{{ old('blindaje') }}"
                ...
                <div>
                    ...
                    <input type="number" id="potencia_motor" name="potencia_motor" value="{{ old('potencia_motor') }}"
                ...
                <div>
                    ...
                    <input type="number" id="velocidad_maxima" name="velocidad_maxima" value="{{ old('velocidad_maxima') }}"
            ...
            <div>
                ...
                <textarea id="descripcion_historica" name="descripcion_historica" rows="4"
                    class="w-full bg-dark-700 border border-military-600 rounded-lg px-4 py-2 text-black placeholder-gray-500 focus:outline-none focus:border-military-400 focus:ring-2 focus:ring-military-400/30 transition-all @error('descripcion_historica') border-red-500 @enderror">{{ old('descripcion_historica') }}</textarea>
                ...
                <div>
                    ...
                    <select id="municiones" name="municiones[]" multiple
                        ...
                        @foreach ($municiones as $municion)
                            <option value="{{ $municion->id }}">{{ $municion->nombre }} ({{ $municion->calibre }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="conflictos" name="conflictos[]" multiple
                        ...
                        @foreach ($conflictos as $conflicto)
                            <option value="{{ $conflicto->id }}">{{ $conflicto->nombre }}</option>
                        @endforeach
                    </select>
                    <p class="mt-2 text-xs text-gray-400">💡 Ctrl+Click para seleccionar múltiples</p>
                </div>
